...
demo_potentials <- function(){
  ...
  plotIris(listIrises,"Potentials,\n Gauss kernel")
  m <- dim(listIrises)[1]
  h <- c(rep(1, m/3), rep(0.5, (m-m/3)))
  kernel = kG
  print(h)
  gamma <- find_gamma(listIrises[,3:5],kernel,h,10)
  for(i in 1:m){
    opaq <- gamma/max(gamma)
    maxgamma <- max(gamma)
    if(gamma[i]>0){ 
      color = adjustcolor(colors[listIrises[i,5]], opaq[i] / 2)
      draw.circle(listIrises[i,3], listIrises[i,4], gamma[i]/maxgamma, 40, border = color, col = color)
  ...
  print(gamma)
  print(h)
  plotIris(listIrises,"Classification map,\n Gauss kernel")
