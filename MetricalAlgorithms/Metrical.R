...
LOOKNN <- function(x){
  ...
  plot(mark,type = "l",main = "LOO(k) ?????????????????? KNN",xlab = "k", ylab="????????????")
...
LOOKWNN <- function(x, k=6){
  ...
  print(mark)
  print(which.min(mark))
  plot(q,mark,type = "l",main = "LOO(k,q) ?????????????????? KWNN",xlab = "q", ylab="????????????")
...
####################################################### 
# ?????????????? ????????
kR <- function(z) 0.5 * (abs(z)<=1) # ??????????????????????????
kT <- function(z) (1 - abs(z))*(abs(z)<=1) #  ??????????????????????
kQ <- function(z) (15/16)*(1 - z^2)^2 * (abs(z)<=1) # ????????????????????????
kE <- function(z) (3/4)*(1-z^2) * (abs(z)<=1) # ????????????????????????
kG <- function(z) dnorm(z) # ??????????????????????

#???????????????????????? ???????? #h_opt: kR - 0.35; kT - 0.35, kQ - 0.35; kE - 0.35; kG - 0.1
...
LOO_parsen <- function(x,K,labels="LOO ?????? ?????????????????????????? ????????"){
  m <- dim(x)[1]
  n <- dim(x)[2] - 1
  ...
  plot(params, mark, type="l",xlab="???????????? ????????", ylab="????????????", main=labels)
...
demonstration <- function(){
  ...
  plotIris(demo_iris, "?????????????????????????? ?????????? ?? ?????????????? kwNN, k=7, q=0.5")
...
#classMapFNN(iris)
#loo <- par(mfrow=c(1,2))
#plotIris(iris, "?????????? ?????????????????????????? KWNN, k=6, q=0.5")
#classMapKWNN(iris)
#plotIris(iris,"?????????? ?????????????????????????? ?????????????????????????? ????????. ???????? ????????????, h=0.1")
#classMapParsen(iris, 0.1, kG)
#par(loo)
#plotIris(iris, "?????????? ?????????????????????????? KNN, k=6")
#classMapKNN(iris)
#knn <- par(mfrow=c(1,2))
#LOOKNN(iris[,3:5])
